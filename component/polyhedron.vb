Module PolyhedronInUnitSphere
    Sub Main()
        ...
        Dim a_tetra = Sqrt(8 / 3)
        Dim area_tetra = Sqrt(3) * a_tetra * a_tetra
        Dim vol_tetra = a_tetra * a_tetra * a_tetra / (6 * Sqrt(2))
        ...
        Dim a_octa = Sqrt(2)
        Dim area_octa = 2 * Sqrt(3) * a_octa * a_octa
        ...
        Dim phi = (1 + Sqrt(5)) / 2
        Dim a_dodeca = 2 / (Sqrt(3) * phi)
        Dim area_dodeca = 3 * Sqrt(25 + 10 * Sqrt(5)) * a_dodeca * a_dodeca
        Dim vol_dodeca = (15 + 7 * Sqrt(5)) / 4 * a_dodeca * a_dodeca * a_dodeca
        ...
        Dim a_icosa = 4 / Sqrt(10 + 2 * Sqrt(5))
        Dim area_icosa = 5 * Sqrt(3) * a_icosa * a_icosa
        Dim vol_icosa = (5 * (3 + Sqrt(5))) / 12 * a_icosa * a_icosa * a_icosa
        polyhedra.Add(("Icosahedron", 20, 12, 30, area_icosa, vol_icosa))
        ' Display results
        For Each p In polyhedra
        Next
        ' Sphere for comparison
        Dim sphereArea = 4 * PI * 1 * 1
        Dim sphereVolume = (4 / 3) * PI * 1 * 1 * 1
        For Each p In polyhedra
        Next
    End Sub
End Module
